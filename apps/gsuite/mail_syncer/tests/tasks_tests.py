...
class GSuiteSignalsTestCase(TestCase):
    ...
    def test_get_error_from_gsuite_http_error(self):
        ...
        http_error = create_http_error(
            status=409, reason=error_message, error=error_message
        )
    ...
    def test_insert_user_pass_if_already_in_group_passing(
        ...
        mocked_insert.side_effect = create_http_error(
            status=409, reason=error_message, error=error_message
        )
    ...
    def test_remove_user_from_group_pass_if_not_subscribed(
        ...
        mocked_remove.side_effect = create_http_error(
            status=404, reason=error_message, error=error_message
        )
