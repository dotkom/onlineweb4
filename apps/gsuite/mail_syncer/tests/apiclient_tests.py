...
class GSuiteAPITestCase(TestCase):
    ...
    def test_insert_email_into_g_suite_group_already_exists_raises(
        ...
        with override_settings(OW4_GSUITE_SYNC=ow4_gsuite_sync):
            insert_email_into_g_suite_group(
                self.domain, group_name, email, suppress_http_errors=True
            )
    ...
    @patch("logging.Logger.debug")
    @patch("apps.gsuite.mail_syncer.utils.setup_g_suite_client", autospec=True)
    def test_remove_g_suite_user_from_g_suite_group_email_address(
    ...
    @patch("logging.Logger.debug")
    @patch("apps.gsuite.mail_syncer.utils.setup_g_suite_client", autospec=True)
    def test_remove_g_suite_user_from_g_suite_group_g_suite_dict(
        self, mocked_g_suite_client, mocked_logger
    ):
        group_name = list(settings.OW4_GSUITE_SYNC.get("GROUPS").keys())[0]
