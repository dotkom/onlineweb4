...
from .serializers import (
    GsuiteAliasCreateOrUpdateSerializer,
    GsuiteGroupCreateSerializer,
    GsuiteGroupUpdateSerializer,
)
...
def insert_email_into_g_suite_group(
    domain: str, group_name: str, email: str, suppress_http_errors=False
):
    """
...
def insert_ow4_user_into_g_suite_group(
    domain: str, group_name: str, ow4_user: User, suppress_http_errors=False
):
    """
...
def get_g_suite_users_for_group(
    domain: str, group_name: str, suppress_http_errors=False
):
    """
    ...
    try:
        ...
        logger.error(
            f"HttpError when requesting user list: {e}",
            extra={"suppress_http_error": suppress_http_errors},
        )
        if not suppress_http_errors:
            raise e
...
def get_g_suite_groups_for_user(domain: str, _user: object, suppress_http_errors=False):
    ...
    try:
        ...
        logger.error(
            f"HttpError when requesting user group membership list: {e}",
            extra={"suppress_http_error": suppress_http_errors},
        )
        if not suppress_http_errors:
            raise e
...
def check_amount_of_members_ow4_g_suite(
    g_suite_members: list, ow4_users: QuerySet, quiet=False
) -> bool:
    """
...
def _get_g_suite_user_from_g_suite_user_list(
    g_suite_users: list, g_suite_email: str
) -> object:
    """
...
def make_gsuite_request(function, suppress_http_errors=False):
    ...
    try:
        ...
        logger.error(
            f"HttpError when inserting into making Gsuite request: {err}",
            extra={"suppress_http_error": suppress_http_errors},
        )
        if not suppress_http_errors:
            raise err
...
def update_gsuite_group(group: GsuiteGroup):
    ...
    def patch_group(directory: Resource):
        return (
            directory.groups()
            .patch(groupKey=group.gsuite_id, body=group_data)
            .execute()
        )
