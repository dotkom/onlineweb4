...
class WebshopOrderLineTests(OIDCTestCase):
    ...
    def test_user_can_create_empty_order_lines(self):
        ...
        self.assertEqual(OrderLine.objects.filter(user=self.user).count(), amount_of_user_order_lines + 1)
...
class WebshopOrderTests(OIDCTestCase):
    ...
    def setUp(self):
        ...
        self.get_order_line = lambda: OrderLine.get_current_order_line_for_user(self.user)
    ...
    def test_user_cannot_create_an_order_with_an_unsupported_size(self):
        ...
        response = self.client.post(self.url, {
            'product': self.product1.id,
            'size': unsupported_size.id,
        }, **self.headers)

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json().get('size'), ['Den gitte størrelsen er ikke tilgjengelig for dette produktet'])

    def test_user_cannot_create_order_for_a_product_with_a_size_without_specifying_size(self):
        G(ProductSize, size='L', product=self.product1)
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json().get('size'), ['Dette produktet krever at du velger en størrelse'])

    def test_user_cannot_create_order_with_size_for_products_without_sizes(self):

        response = self.client.post(self.url, {
            'product': self.product1.id,
            'size': self.product_size_l.id,
        }, **self.headers)

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json().get('size'), ['Du kan ikke velge en størrelse for dette produktet'])
    ...
    def test_user_cannot_add_the_same_product_twice(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json().get('non_field_errors'), ['Dette produktet er allerede i handlevognen din'])
    ...
    def test_user_cannot_create_orders_with_inactive_products(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json().get('product'), ['Produktet er ikke lenger aktivt og kan ikke kjøpes'])
    ...
    def test_user_cannot_create_orders_with_products_past_their_deadline(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json().get('product'), ['Tidsfristen for å kjøpe produktet har utgått'])
    ...
    def test_user_cannot_add_products_with_not_enough_stock(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json().get('product'), ['Det er ikke flere varer igjen av produktet'])

    def test_user_cannot_create_order_with_product_with_not_enough_stock_of_selected_size(self):
