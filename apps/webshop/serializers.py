...
class ProductReadOnlySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = (
            'id', 'category', 'name', 'slug', 'short', 'description', 'images', 'price', 'stock', 'deadline',
            'active', 'product_sizes', 'related_products',
        )
        read_only = True
...
class OrderCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_product(self, product: Product):
        ...
        if not product.active:
            raise serializers.ValidationError(f'Produktet er ikke lenger aktivt og kan ikke kjøpes')

        if timezone.now() > product.deadline:
            raise serializers.ValidationError(f'Tidsfristen for å kjøpe produktet har utgått')

        if product.stock == 0:
            raise serializers.ValidationError(f'Det er ikke flere varer igjen av produktet')
    ...
    def validate_size(self, size: ProductSize):
        ...
        if product_id:
            ...
            if not has_size and size:
                raise serializers.ValidationError('Du kan ikke velge en størrelse for dette produktet')

            if has_size and not size:
                raise serializers.ValidationError('Dette produktet krever at du velger en størrelse')

            if has_size and size not in allowed_sizes:
                raise serializers.ValidationError(f'Den gitte størrelsen er ikke tilgjengelig for dette produktet')

            if not product.enough_stock(quantity, size):
                if not size:
                    raise serializers.ValidationError(f'Det er ikke flere varer igjen av dette produktet')
                else:
    ...
    def validate_order_line(self, order_line: OrderLine):
        ...
        if order_line.user != request.user:
            raise serializers.ValidationError(f'Du kan ikke legge varer til andre brukeres handlevogn')
    ...
    def validate(self, data):
        ...
        if product in products_in_order_line:
            raise serializers.ValidationError('Dette produktet er allerede i handlevognen din')
    ...
    class Meta:
        model = Order
        fields = ('id', 'product', 'price', 'quantity', 'size', 'is_valid', 'order_line',)
...
class OrderLineCreateSerializer(OrderLineReadOnlySerializer):
    ...
    class Meta(OrderLineReadOnlySerializer.Meta):
        ...
        read_only_fields = (
            'id', 'datetime', 'paid', 'stripe_id', 'delivered', 'subtotal', 'is_valid',
        )
