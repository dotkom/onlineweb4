...
class Article(models.Model):
    ...
    image = models.ForeignKey(
        ResponsiveImage, null=True, default=None, blank=True, on_delete=SET_NULL
    )
    video = models.CharField(
        _("vimeo id"), max_length=200, blank=True, validators=[vimeo_id_validator]
    )
    created_date = models.DateTimeField(
        _("opprettet-dato"), auto_now_add=True, editable=False
    )
    ...
    def get_absolute_url(self):
        return reverse(
            "article_details", kwargs={"article_id": self.id, "article_slug": self.slug}
        )
