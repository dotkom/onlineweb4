...
class EventPDF(object):
    ...
    def create_attendees_table_data(self):
        ...
        for attendee in self.attendees:
            ...
            if attendee.note:
                self.attendee_table_data.append((create_body_text('Notat for %s: ' % attendee.user.first_name + attendee.note),))
                i += 1
    ...
    def create_waiters_table_data(self):
        ...
        for attendee in self.waiters:
            ...
            if attendee.note:
                self.waiters_table_data.append((create_body_text('Notat for %s: ' % attendee.user.first_name + attendee.note),))
                i += 1
