...
class EventPDF(object):
    ...
    def __init__(self, event):
        self.event = event
        attendees = event.attendance_event.attending_attendees_qs
        self.attendees = sorted(attendees, key=lambda attendee: attendee.user.last_name)
    ...
    def create_reservees_table_data(self):
        for reservee in self.reservees:
            self.reservee_table_data.append(
                (create_body_text(reservee.name), create_body_text(reservee.note))
            )
