...
class CreateAttendeeTestCase(OIDCTestCase):
    ...
    def test_user_cannot_attend_a_non_attendance_event(self, _):
        ...
        response = self.client.post(
            self.url,
            {"event": event_without_attendance.id, **self.recaptcha_arg},
            **self.headers,
        )
    ...
    def test_guest_user_can_attend_event_with_guest_attendance(self, _):
        ...
        response = self.client.post(
            self.url, {"event": self.event.id, **self.recaptcha_arg}, **self.headers
        )
    ...
    def test_guest_user_cannot_attend_other_users_to_events(self, _):
        ...
        response = self.client.post(
            self.url,
            {"event": self.event.id, "user": other_user.id, **self.recaptcha_arg},
            **self.headers,
        )
        ...
        self.assertEqual(
            ...
            ["Du kan ikke melde andre brukere på arrangementer!"],
        )
    ...
    def test_user_can_change_show_as_attending_event_after_registering(self, _):
        ...
        patch_response = self.client.patch(
            self.id_url(attendee_id), {"show_as_attending_event": True}, **self.headers
        )
    ...
    def test_user_can_set_allow_pictures_on_registration(self, _):
        ...
        response = self.client.post(
            self.url,
            {"event": self.event.id, "allow_pictures": True, **self.recaptcha_arg},
            **self.headers,
        )
    ...
    def test_user_can_change_allow_pictures_after_registering(self, _):
        ...
        patch_response = self.client.patch(
            self.id_url(attendee_id), {"allow_pictures": True}, **self.headers
        )
    ...
    def test_guest_user_cannot_attend_event_with_guest_attendance_without_captcha(self):
        ...
        response = self.client.post(self.url, {"event": self.event.id}, **self.headers)
    ...
    def test_user_cannot_attend_twice(self, _):
        ...
        self.assertEqual(
            ...
            ["Feltene event, user må gjøre et unikt sett."],
        )
    ...
    def test_user_cannot_attend_event_after_registration_end(self, _):
        ...
        self.assertEqual(
            response.json().get("non_field_errors"),
            ["Påmeldingen er ikke lenger åpen."],
        )
    ...
    def test_payment_is_generated_on_attendance(self, _):
        ...
        attendee_response = self.client.get(
            self.id_url(attendee_id), {"event": self.event.id}, **self.headers
        )
...
class AttendAPITestCase(OIDCTestCase):
    ...
    def test_unknown_rfid(self):
        response = self.client.post(
            self.url, {"event": self.event.id, "rfid": "fake_rfid"}, **self.headers
        )
    ...
    def test_api_does_not_try_to_get_user_by_rfid_empty_string(self):
        ...
        response = self.client.post(
            self.url, {"event": self.event.id, "rfid": ""}, **self.headers
        )
    ...
    def test_save_rfid_give_no_username_gives_useful_error_message(self):
        ...
        response = self.client.post(
            self.url,
            json.dumps({"event": self.event.id, "rfid": self.attendee1.user.rfid}),
            content_type="application/json",
            **self.headers,
        )
    ...
    def test_attend_without_event_returns_correct_status(self):
        response = self.client.post(
            self.url, {"username": self.attendee1.user.username}, **self.headers
        )
