...
class EventTest(TestCase):
    def setUp(self):
        ...
        self.attendance_event: AttendanceEvent = G(AttendanceEvent, event=self.event)
        self.registration: Registration = G(Registration, attendance=self.attendance_event, max_capacity=2)
        self.user: User = G(User, username='ola123', ntnu_username='ola123ntnu',
    ...
    def testMommyAttended(self):
        G(Attendee, event=self.attendance_event, registration=self.registration, user=self.user, attended=True)
