...
class Event(models.Model):
    ...
    def is_eligible_for_signup(self, user):
        ...
        # Registration closed
        if datetime.now(timezone(settings.TIME_ZONE)) > self.attendance_event.registration_end:
        ...
        if active_marks > 0:
            ...
            if postponed_registration_start > datetime.now(timezone(settings.TIME_ZONE)):
        ...
        if 'offset' in response and response['offset'] > datetime.now(timezone(settings.TIME_ZONE)):
            return response 
        ...
        if datetime.now(timezone(settings.TIME_ZONE)) < self.attendance_event.registration_start:
...
class FieldOfStudyRule(Rule):
    ...
    def satisfied(self, user, registration_start):
        ...
        if (self.field_of_study == user.field_of_study):
            ...
            if offset_datetime <= datetime.now(timezone(settings.TIME_ZONE)):
