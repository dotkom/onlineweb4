...
def login(request):
    ...
    if request.method == "POST":
        ...
        if form.login(request):
            messages.success(
                request, _("Du er nå logget inn."), extra_tags="data-dismiss"
            )
            if redirect_url:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    log.error(
                        'Failed to register token for "%s" due to "%s"'
                        % (request.user, ie)
                    )
                ...
                messages.success(
                    request,
                    _(
                        "Registreringen var vellykket. Se tilsendt epost for verifiseringsinstrukser."
                    ),
                )
        ...
        if user_activated:
            ...
            messages.success(
                ...
                _(
                    ...
                    'Kikk rundt på "Min Side" for å oppdatere profilinnstillinger.'
                )
                % user.username,
            )
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if len(emails) == 0:
                    messages.error(
                        request, _("Denne eposten er ikke registrert i våre systemer.")
                    )
                    return HttpResponseRedirect("/")
...
def set_password(request, token=None):
    ...
    if request.user.is_authenticated:
        ...
        if rt and rt.is_valid:
            if request.method == "POST":
                ...
                if form.is_valid():
                    ...
                    messages.success(
                        request,
                        _(
                            'Passordgjenoppretting gjennomført for "%s". '
                            + "Du kan nå logge inn."
                        )
                        % user.username,
                    )
                    log.info(
                        'User "%s" successfully recovered password.' % request.user
                    )
                    return HttpResponseRedirect("/")
                else:
                    messages.error(
            ...
            return render(
                request, "auth/set_password.html", {"form": form, "token": token}
            )
        log.debug(
            "User %s failed to recover password with token %s." % (request.user, rt)
        )
        messages.error(
            request,
