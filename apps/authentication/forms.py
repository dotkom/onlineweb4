...
class LoginForm(forms.Form):
    username = forms.CharField(
        widget=forms.TextInput(), label=_("Brukernavn"), max_length=50
    )
    ...
    def clean(self):
        ...
        user = auth.authenticate(
            username=username, password=self.cleaned_data["password"]
        )
...
class RegisterForm(forms.Form):
    ...
    repeat_password = forms.CharField(
        widget=forms.PasswordInput(), label=_("Gjenta passord")
    )
    ...
    def clean(self):
        ...
        if self.is_valid():
            ...
            if User.objects.filter(username=username).count() > 0:
                self._errors["username"] = self.error_class(
                    [_("Brukernavnet er allerede registrert.")]
                )
            if not re.match("^[a-zA-Z0-9_-]+$", username):
                self._errors["username"] = self.error_class(
                    [
                        _(
                            "Ditt brukernavn inneholdt ulovlige tegn. Lovlige tegn: a-Z 0-9 - _"
                        )
                    ]
                )
            ...
            if Email.objects.filter(email=email).count() > 0:
                self._errors["email"] = self.error_class(
                    [_("Det eksisterer allerede en bruker med denne epostadressen.")]
                )
...
class ChangePasswordForm(forms.Form):
    new_password = forms.CharField(
        widget=forms.PasswordInput(), label=_("Nytt passord")
    )
    repeat_password = forms.CharField(
        widget=forms.PasswordInput(), label=_("Gjenta passord")
    )
