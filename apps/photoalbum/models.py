...
class Album(models.Model):
    ...
    title = models.CharField(
        "Tittel", blank=False, null=False, max_length=128, unique=True
    )
    ...
    tags = TaggableManager(
        help_text="En komma eller mellomrom-separert liste med tags.", blank=True
    )
    ...
    def get_previous_photo(self, photo: "Photo") -> "Photo":
        ordered_photos = self.photos.order_by("-created_date")
        previous_photo = ordered_photos.filter(
...
class Photo(models.Model):
    album = models.ForeignKey(
        to=Album, related_name="photos", on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    relative_id = models.IntegerField(
        "Relativ id for album", null=False, blank=True, editable=False
    )
    ...
    photographer_name = models.CharField(
        "Fotografnavn", max_length=100, null=False, blank=True
    )
