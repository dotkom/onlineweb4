...
class PaymentReminder(Task):
    ...
    def send_missed_payment_mail(payment):
        subject = _(u"Betalingsfrist utgått: ") + payment.description()
        message = _(u"Hei, du har ikke betalt for følgende arrangement: ") + payment.description()
        message += _(u"Fristen har utgått, og du har mistet plassen din på arrangementet")
        message += _(u"\nFor mer info om arrangementet se:")
        message += "\n" + str(settings.BASE_URL + payment.content_object.event.get_absolute_url())
    ...
    def set_marks(payment):
        ...
        mark.description = _(u"Du har fått en prikk fordi du ikke har betalt for et arrangement.")
        mark.save()
